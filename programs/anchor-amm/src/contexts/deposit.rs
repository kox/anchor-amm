...
use anchor_spl::{associated_token::AssociatedToken, token::{mint_to, transfer, MintTo, Token, Transfer}, token_interface::{Mint, TokenAccount}};
use constant_product_curve::ConstantProduct;

use crate::{assert_non_zero, assert_not_expired, assert_not_locked, AmmError, Config};
...
pub struct Deposit<'info> {
    ...
    // now we will have the mint token accounts
    pub x_mint: Box<InterfaceAccount<'info, Mint>>, 
    pub y_mint: Box<InterfaceAccount<'info, Mint>>,

    // now we are going to define the lp_mint. IT will contain the token information for our LP s
    #[account(
        init_if_needed,
        payer = payer,
    ...
    pub x_user: Box<Account<'info, TokenAccount>>,
    #[account(
        mut,
        associated_token::mint = config.y_mint,
        associated_token::authority = payer,
    )]
    pub y_user: Box<Account<'info, TokenAccount>>,
    ...
    pub lp_user: Box<Account<'info, TokenAccount>>,
...
impl<'info> Deposit<'info> {
    ...
    pub fn deposit(
        ...
        let (x,y) = match self.lp_mint.supply == 0 && self.x_vault.amount == 0 && self.y_vault.amount == 0 {
            true => (x_max, y_max),
            false => {
    ...
    pub fn deposit_tokens(
        ...
        let (from, to) = match is_x {
            true => (self.x_user.to_account_info(), self.x_vault.to_account_info()),
            false => (self.y_user.to_account_info(), self.y_vault.to_account_info())
        };
